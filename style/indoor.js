var indoorStyle = {
    "name": "Indoor",
    "active": false,
    "extrusion": true,
    "layers": [
         // this layer is almost invisible in rendering but is needed because mapbox.gl doesnt check all layer for level if all layer are set invisible
        {
            "id": "justForRendering",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "line",
            "filter": ["all", ["any", ["==", "railway", "rail"],
                ["==", "railway", "light_rail"]
            ]],
            "layout": {
                //  "visibility": "none", <<<< this is the difference to all other layer
                "line-cap": "square"
            },
            "paint": {
                "line-color": "#D6D6D6",
                "line-width": 1
            },
        },
        {
            "id": "room",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["any", ["==", "indoor", "room"],
                ["==", "room", "yes"],
                ["has", "shop"]
            ]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "#D6D6D6"
            }
        }, 
        {
            "id": "corridor",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", 
                ["==", "room", "corridor"],
                ["==", "indoor", "corridor"],
                ["==", "indoor", "area"],
                ["==", "building_part", "floor"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "white"
            }
        }

        ,
          {
            "id": "room_outline",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["any", 
                ["!=", "room", "corridor"],
                ["!=", "indoor", "corridor"],
                ["!=", "indoor", "area"],
                ["!=", "building_part", "floor"]
                           
            ]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "line-color": "#7E7760",
                "line-width": 2

            }
        }, 
        
        /*
        ,
       
        {
            "id": "room_access",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["all", ["==", "indoor", "room"],
                ["any", ["==", "access", "private"],
                    ["==", "access", "no"]
                ]
            ]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "red"
            }
        }, {
            "id": "stairwell",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["==", "stairwell", "flight_of_stairs"],
                ["==", "stairwell", "yes"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "#C4C8CC",

            }
        }, {
            "id": "stairs",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "line",
            "minzoom": 17,
            "filter": ["all", ["all", ["==", "highway", "steps"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                'line-color': '#7e7f80',
                'line-width': {
                    "stops": [
                        [18, 9],
                        [19, 18],
                        [20, 32]
                    ]
                },
                'line-dasharray': [0.05, 0.1] // 0.2,0.4 first number line second number gap
            }
        },
       {
            "id": "platform",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["any", ["any", ["==", "public_transport", "platform"],
                ["==", "railway", "platform"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "#C1C1C0",
                "fill-outline-color": "#7e7f80",
            }
        }, {
            "id": "fence",
            "minzoom": 19,
            "type": "line",
            "source": "indoor_source",
            "source-layer": "line",
            "filter": ["all", ["any", ["==", "barrier", "fence"],
                ["==", "barrier", "railing"],
                ["==", "barrier", "wall"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                'line-color': 'black',
                'line-width': 1.5

            }
        }, {
            "id": "facilities",
            "type": "fill",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["==", "amenity", "toilets"],
                ["==", "room", "toilets"],
                ["==", "amenity", "police"],
                ["==", "public_transport", "waiting_room"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "fill-color": "#066C9E",
                "fill-outline-color": "yellow"
            }
        },  {
            "id": "room_outline",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["any", ["==", "indoor", "room"],
                ["==", "room", "yes"],
                ["==", "amenity", "toilets"],
                ["==", "room", "toilets"],
                ["==", "amenity", "police"],
                ["==", "public_transport", "service_center"],
                ["==", "public_transport", "service_point"],
                ["==", "public_transport", "waiting_room"],
                ["==", "shop", "ticket"],
                ["==", "amenity", "luggage_locker"],
                ["!=", "room", "corridor"],
                ["!=", "indoor", "corridor"],
                ["!=", "indoor", "area"],
                ["!=", "building_part", "floor"]
            ]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "line-color": "#7E7760",
                "line-width": 2

            }
        }, {
            "id": "stairwell_outline",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["==", "stairwell", "flight_of_stairs"],
                ["==", "stairwell", "yes"]
            ]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "line-color": "#7e7f80",
                "line-width": 2
            }
        }, {
            "id": "stairlanding_outline",
            "type": "line",
            "source": "indoor_source",
            "source-layer": "polygon",
            "filter": ["all", ["any", ["==", "stairwell", "stair_landing"]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                "line-color": "#7e7f80",
                "line-width": 2
            }
        },      
        {
            "id": "footway",
            "minzoom": 18,
            "type": "line",
            "source": "indoor_source",
            "source-layer": "line",
            "filter": ["all", ["any", ["==", "highway", "footway"]]],
            "layout": {
                "visibility": "none",
                "line-cap": "round",
            },
            "paint": {
                'line-color': '#ddd',
                'line-width': 1,
                'line-dasharray': [1, 3] // 0.2,0.4 first number line second number gap
            }
        }, {
            "id": "door",
            "minzoom": 18,
            "type": "line",
            "source": "indoor_source",
            "source-layer": "line",
            "filter": ["all", ["all", ["has", "door"]]],
            "layout": {
                "visibility": "none"
            },
            "paint": {
                'line-color': '#EDEDED',
                'line-width': 3
            }
        },
        // ++++++++++++++ START LABEL ++++++++++++++++++++++
        {
            "id": "text_shop",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["all", ["==", "room", "shop"],
                ["has", "name"]
            ]],
            "layout": {
                "visibility": "none",
                "text-offset": [0, 0.5],
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-anchor": "center",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "bench",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["all", ["==", "amenity", "bench"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "wartebereich.small",
                "icon-size": 0.8
            }
        }, {
            "id": "label_waiting_room",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "public_transport", "waiting_room"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "wartebereich.small",
                "icon-size": 0.8
            }
        }, {
            "id": "atm",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["all", ["==", "amenity", "atm"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "ec_atm.small",
                "icon-size": 0.8
            }
        }, {
            "id": "ticket_validator",
            "minzoom": 19,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["all", ["==", "vending", "ticket_validator"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "fahrkartenentwerter.small",
                "icon-size": 0.8
            }
        }, {
            "id": "label_bank",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "bank"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "bank-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_books",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "shop", "books"],
                ["==", "shop", "newsagent"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "library-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "label_florist",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["all", ["==", "shop", "florist"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "garden-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "label_bakery",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "bakery"],
                ["==", "shop", "bakery"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "bakery-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "label_restaurant",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "restaurant"],
                ["==", "indoor", "restaurant"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "restaurant-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_fast_food",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "fast_food"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "fast-food-15",
                "icon-size": 1.2,

                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_cafe",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "cafe"],
                ["==", "shop", "coffee"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "cafe-15",
                "icon-size": 1.2,

                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "label_car_rental",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["all", ["==", "amenity", "car_rental"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "car-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_pharmacy",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["all", ["==", "amenity", "pharmacy"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "hospital-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_clothes",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["all", ["==", "shop", "clothes"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "clothing-store-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "entrance",
            "minzoom": 16,
            "type": "symbol",
            "source": "indoor_source",
            "source-layer": "point",
            "filter": ["any", ["any", ["==", "entrance", "yes"],
                    ["==", "entrance", "main"]
                ],
                ["any", ["has", "ref"]]
            ],
            "layout": {
                "visibility": "none",
                "icon-image": "entrance",
                "icon-allow-overlap": true,
                "icon-size": 2,
                "text-field": "{ref}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0.4, 1.2],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            18 //11
                        ],
                        [
                            20,
                            22 //16
                        ]
                    ]
                }
            },
            "paint": {
                "text-color": "black",
                "text-halo-width": 2,
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
        }, {
            "id": "label_information",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "tourism", "information"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "information-15",
                "icon-size": 1.2,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 0.5],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "label_luggage_locker",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "luggage_locker"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "schliessfach.small",
                "icon-size": 0.8
            }
        }, {
            "id": "public_transport_tickets",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["all", ["==", "amenity", "vending_machine"],
                ["==", "vending", "public_transport_tickets"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "fahrkarten.small",
                "icon-size": 0.8
            }
        }, {
            "id": "label_toilet",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "amenity", "toilets"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "wc.small",
                "icon-size": 0.8
            }
        }, {
            "id": "information_point",
            "minzoom": 17,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["any", ["==", "tourism", "information"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "information-15",
                "icon-size": 0.4
            }
        }, {
            "id": "elevatorpoint",
            "minzoom": 17,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["any", ["==", "highway", "elevator"]]],
            "layout": {
                "visibility": "none",
                "icon-image": "aufzug.small",
                "icon-size": //0.8
                            {
                               "stops": [
                                    [17, 0.5], [18, 0.8], [19, 0.9],
                                    [20, 1]
                                        ]
                            }
            }
        }, {
            "id": "service_symbol",
            "minzoom": 18,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "label",
            "filter": ["all", ["any", ["==", "public_transport", "service_center"],
                ["==", "public_transport", "service_point"],
                ["==", "shop", "ticket"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "i.small",
                "icon-size": 0.8,
                "text-field": "{name}",
                "text-font": ["Open Sans Regular", "Arial Unicode MS Regular"],
                "text-max-width": 4,
                "text-offset": [0, 1],
                "text-anchor": "top",
                "text-size": {
                    "base": 1.4,
                    "stops": [
                        [
                            18,
                            11
                        ],
                        [
                            20,
                            16
                        ]
                    ]
                }
            }
        }, {
            "id": "local_ref1",
            "minzoom": 17,
            "type": "symbol",
            "source": "indoor_source",

            "source-layer": "point",
            "filter": ["all", ["all", ["==", "public_transport", "stop_position"],
                ["has", "local_ref"]
            ]],
            "layout": {
                "visibility": "none",
                "icon-image": "gleis_{local_ref}.small",
                "icon-size": 0.7
            }
        }, { //------------------ S bahn label
            "interactive": true,
            "minzoom": 14,
            "maxzoom": 19,
            "layout": {

                "icon-image": "s.small",
                "text-offset": [
                    0,
                    0.6
                ],
                "text-field": "{name_en}",
                "text-font": [
                    "Open Sans Semibold",
                    "Arial Unicode MS Bold"
                ],
                "text-max-width": 8,
                "text-anchor": "top",
                "text-size": 15,
                "icon-size": 0.8
            },
            "filter": ["all", ["==", "$type", "Point"],
                ["==", "maki", "rail-light"],
                ["==", "network", "de-s-bahn"]
            ],
            "type": "symbol",
            "source": "mapbox",
            "id": "railstation_light_label",
            "paint": {

                "text-color": "black",
                "text-halo-width": 3, // wert 4 verworfen
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1

            },
            "source-layer": "rail_station_label"
        }, { //------------------ underground label
            "interactive": true,
            "minzoom": 14,
            "maxzoom": 19,
            "layout": {
                "icon-image": "u.small",
                "text-offset": [
                    0,
                    0.6
                ],
                "text-field": "{name_en}",
                "text-font": [
                    "Open Sans Semibold",
                    "Arial Unicode MS Bold"
                ],
                "text-max-width": 8,
                "text-anchor": "top",
                "text-size": 15,
                "icon-size": 0.7
            },
            "filter": ["all", ["==", "$type", "Point"],
                ["==", "maki", "rail-metro"],
                ["==", "network", "de-u-bahn"]
            ],
            "type": "symbol",
            "source": "mapbox",
            "id": "railstation_underground_label",
            "paint": {
                "text-color": "black",
                "text-halo-width": 3, 
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
            "source-layer": "rail_station_label"
        }, { //------------------ bahn label
            "interactive": true,
            "minzoom": 12,
            "maxzoom": 19,
            "layout": {
                "icon-image": "rail-light-15", // db_logo.small
                "text-offset": [
                    0,
                    0.6
                ],
                "text-field": "{name_en}",
                "text-font": [
                    "Open Sans Semibold",
                    "Arial Unicode MS Bold"
                ],
                "text-max-width": 8,
                "text-anchor": "top",
                "text-size": 15,
                "icon-size": 1
            },
            "filter": ["all", ["==", "$type", "Point"],
                ["==", "maki", "rail"],
                ["==", "network", "rail"]
            ],
            "type": "symbol",
            "source": "mapbox",
            "id": "railstation_label",
            "paint": {
                "text-color": "black",
                "text-halo-width": 3, 
                "text-halo-color": "rgba(255,255,255,0.75)",
                "text-halo-blur": 1
            },
            "source-layer": "rail_station_label"
        }
        */
    ]
}